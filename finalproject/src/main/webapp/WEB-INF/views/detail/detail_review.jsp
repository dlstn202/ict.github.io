...
function setRating(re_idx, rating) {
    $("#re_review_grade_new" + re_idx).val(rating);
}
         ...
         <!-- --------------------------------------- -->
   		<c:forEach var="vo" items="${ review_list }">
			    ...
			    <div id="review_bord">
				   	...
				   	<hr>
					<div class="review_writer" id="review_writer">${vo.pl_mem_id}</div>
					
					<div class="review_regdate" id="re_regdate${vo.re_idx}">
						${fn:substring(vo.re_regdate, 0, 10)}
						<c:if test="${user.pl_mem_idx eq vo.pl_mem_idx or user.pl_mem_grade eq '관리자'}">
							<span class="del_close" >
								...
								<span onclick="delRe(${vo.re_idx});">x</span>
							</span>
						</c:if>
					...
					<div class="review_write"> 						
						<div> 
							...
							<div class="review_star2" id="re_review_star${vo.re_idx}">${vo.re_star}</div>
							<div class="review_content" id="re_content${vo.re_idx}">${vo.re_content}</div>
						</div>
